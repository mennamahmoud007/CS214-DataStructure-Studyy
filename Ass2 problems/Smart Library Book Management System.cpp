...
int main()
{
    ...
    bstRandom.search(bstRandom.root, 999); // non-existing  
        cout << "\nAVL Search:\n";
    avlRandom.search(99); // existing
    avlRandom.search(999); // non-existing
    ...
    bstSorted.search(bstSorted.root, 999); // non-existing  
        cout << "\nAVL Search:\n";  
    avlSorted.search(100); // existing
    avlSorted.search(999); // non-existing


    // AVL maintains a smaller height and fewer search steps, 
    // especially when data is inserted in sorted order.
